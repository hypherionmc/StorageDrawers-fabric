...
import me.hypherionmc.storagedrawers.item.*;
...
public class UpgradeData extends TileDataShim
{
    ...
    public void read (NbtCompound tag) {
        ...
        NbtList tagList = tag.getList("Upgrades", Constants.NBT.TAG_COMPOUND);
        for (int i = 0; i < tagList.size(); i++) {
            NbtCompound upgradeTag = tagList.getCompound(i);

            int slot = upgradeTag.getByte("Slot");
            upgrades[slot] = ItemStack.fromNbt(upgradeTag);
        }
    ...
    public NbtCompound write (NbtCompound tag) {
        ...
        for (int i = 0; i < upgrades.length; i++) {
            if (!upgrades[i].isEmpty()) {
                NbtCompound upgradeTag = upgrades[i].writeNbt(new NbtCompound());
